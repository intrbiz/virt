...
public class NetworkPIRouter extends Router<VirtDashApp>
{    
    @Get("/account/name/:name/")
    @JSON
    @WithDataAdapter(VirtDB.class)
    public List<Network> listNetworks(VirtDB db, @CurrentPrincipal User user, String accountName)
    {
        Account account = notNull(user.getOwnedAccounts().stream().filter(a -> a.getName().equals(accountName)).findFirst().orElse(null));
        return db.getNetworksForAccount(account.getId());
    }
}
