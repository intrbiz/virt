...
public class GuestFilter extends Router<VirtHostApp>
{
    ...
    @Before
    @Any(value="/(?:metadata|status|meta-data|user-data|vendor-data|network-config).*", regex=true)
    ...
    public void filterMachine(VirtDB db)
    {
        ...
        require(cfgMac != null, "Cannot resolve MAC address for: " + request().getRemoteAddress());
        // lookup the machine
